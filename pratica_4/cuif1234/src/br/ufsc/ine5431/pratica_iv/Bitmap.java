...
public class Bitmap {
	...
	// Construtor para gerar objeto Bitmap a partir do arquivo bitmap de nome
	// "filename"
	...
	// Método que lê os pixels do arquivo Cuif1 e gera raster, incluindo no array de
	// bytes do arquivo
	...
	// Método que lê os pixels do arquivo Cuif2 e gera raster, incluindo no array de
	// bytes do arquivo
	...
	// Método que lê os pixels do arquivo Cuif3 e gera raster, incluindo no array de
	// bytes do arquivo
	private void cuif3toRaster(byte[] cuiffile, int index) throws IOException {
		...
		// Resto da operação é a mesma do CUIF2, pois o dado descompactado é o array
		// YCbCr
		cuif2toRaster(data, 0);
	}

	// Método que lê os pixels do arquivo Cuif4 e gera raster, incluindo no array de
	// bytes do arquivo
	private void cuif4toRaster(byte[] cuiffile, int index) {
		...
		// Dado descompactado em "data" � o array RGB, basta gerar raster e coloca no
		// array de bytes represenando arquivo
		readRaster(data, 0);
	}
	...
	private void readBMPHeader(byte[] bitmapfile) throws Exception {
		...
		size_of_bitmap_file = (bitmapfile[2] & 0xff) | (bitmapfile[3] & 0xff) << 8 | (bitmapfile[4] & 0xff) << 16
		...
		offset_to_start_image = (bitmapfile[10] & 0xff) | (bitmapfile[11] & 0xff) << 8 | (bitmapfile[12] & 0xff) << 16
				| (bitmapfile[13] & 0xff) << 24;
		size_of_header = (bitmapfile[14] & 0xff) | (bitmapfile[15] & 0xff) << 8 | (bitmapfile[16] & 0xff) << 16
				| (bitmapfile[17] & 0xff) << 24;
		...
		horizontal_pixels_per_meter = (bitmapfile[38] & 0xff) | (bitmapfile[39] & 0xff) << 8
				| (bitmapfile[40] & 0xff) << 16 | (bitmapfile[41] & 0xff) << 24;
		vertical_pixels_per_meter = (bitmapfile[42] & 0xff) | (bitmapfile[43] & 0xff) << 8
				| (bitmapfile[44] & 0xff) << 16 | (bitmapfile[45] & 0xff) << 24;
		number_of_colors = (bitmapfile[46] & 0xff) | (bitmapfile[47] & 0xff) << 8 | (bitmapfile[48] & 0xff) << 16
				| (bitmapfile[49] & 0xff) << 24;
		number_of_important_colors = (bitmapfile[50] & 0xff) | (bitmapfile[51] & 0xff) << 8
				| (bitmapfile[52] & 0xff) << 16 | (bitmapfile[53] & 0xff) << 24;
	}
